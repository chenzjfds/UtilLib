...
/**
 * creaed by  zhijunchen on 2019/5/23
 */
public class Zip4jUtil {
    ...
    public static String zip(String src, String dest, boolean isCreateDir, String passwd) {
        Log.i(TAG,"ZIP START");
        Log.i(TAG, " src=" + src + "  dest=" + dest + "pwd=" + passwd);
        if (!dest.endsWith("/")) {
            dest += "/";
        }
     ...
     * 如果以路径分隔符(File.separator)结尾,则视为目录,压缩文件名取源文件名,以.zip为后缀,否则视为文件名.
     *
     * @param data   要压缩的文件或文件夹路径
     * @param dest   压缩文件存放路径
     * @param passwd 压缩使用的密码
     * @return 最终的压缩文件存放的绝对路径, 如果为null则说明压缩失败.
     */
    public static String zip(byte[] data, String dest, String name, String passwd) {
        ZipParameters parameters = new ZipParameters();
        parameters.setCompressionMethod(Zip4jConstants.COMP_DEFLATE);           // 压缩方式
        parameters.setCompressionLevel(Zip4jConstants.DEFLATE_LEVEL_NORMAL);    // 压缩级别
        if (!TextUtils.isEmpty(passwd)) {
            ...
            Log.i(TAG, "ZIP internal filename=" + name);
            parameters.setFileNameInZip(name);
        }
        try {
            Log.i(TAG, "ZIP internal filelength=" + data.length);
            ZipFile zipFile = new ZipFile(dest);
