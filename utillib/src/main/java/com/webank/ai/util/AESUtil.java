...
//            byte[] enc = Base64Decoder.decodeToBytes(encrypted);
            ...
            LogUtil.i("aesutil"," key="+key +" ency="+encrypted+" value="+data);
            return data;
        } catch (Exception e) {
            e.printStackTrace();
            LogUtil.e("aesutil","property文件解析失败 e="+e.getMessage());
        }
        return null;
    }
    ...
    private static byte[] decrypt(String key, byte[] encrypted) throws Exception {
        ...
        SecretKeySpec skeySpec = new SecretKeySpec(raw, AES);
        Cipher cipher = Cipher.getInstance(CBC_PKCS5_PADDING);
        cipher.init(DECRYPT_MODE, skeySpec, new IvParameterSpec(new byte[cipher.getBlockSize()]));
